...
from sklearn.metrics import ConfusionMatrixDisplay # Felipe C- Para plotar a matrix de confusão
import matplotlib.pyplot as plt # Felipe C- Para plotar a matrix de confusão
...
def _print_train_results(classifier_name, classifier, regressors, response, regressor_names, leave_one_out):
    ...
    displayConfMatrix = ConfusionMatrixDisplay(confusion, display_labels=labels)# Felipe C- Para plotar a matrix de confusão
    displayConfMatrix.plot(cmap='Blues')# Felipe C- Para plotar a matrix de confusão
    plt.show()# Felipe C- Para plotar a matrix de confusão
    ...
    if classifier_name in ["Random Forest"]:
        des_fun = classifier.predict_proba(regressors)[:, classifier.classes_ == 1] #FC usado para dar a resposta em prob do modelo de Random Forest
    else:
        des_fun = classifier.decision_function(regressors) # FC usado para dar a distância do hiperplano da função SVM
    ...
    #Felipe 29/05/23
                #Obtendo coeficientes da regressão logística
    '''if classifier_name == "Logistic Regression":
        MESSAGES.AddMessage("Coeficientes: {}".format(classifier.coef_))
        MESSAGES.AddMessage("Intercepto: {}".format(classifier.intercept_))

        #Criando um gráfico de barras
        plt.bar(regressors, classifier.coef_)
    ...
    #Felipe 29/05/23
        #Obtendo coeficientes do Random forest
    if classifier_name == "Random Forest":
        MESSAGES.AddMessage("Importancias: {}".format(classifier.feature_importances_))

        #Agora criando gráfico de barras
        importances = classifier.feature_importances_ #Criando variável
        indices = np.argsort(importances)[::-1] #Ordenando
        plt.figure(figsize=(10, 6))
        plt.title("Importância dos Recursos")
        plt.bar(range(regressors.shape[1]), importances[np.argsort(importances)], align='center')
        plt.xticks(range(regressors.shape[1]), train_regressors_name[indices], rotation=90)
        plt.xlabel("Mapas evidenciais")
...
def execute(self, parameters, messages):
    ...
    if classifier_name == "Adaboost":
        ...
        classifier = RandomForestClassifier(n_estimators=num_estimators, criterion='gini', max_depth=max_depth, min_samples_split=2,
                               min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features='sqrt', #mudei para 'sqrt'
...
#Felipe 29/05/23
    if classifier_name == "Logistic Regression":
        MESSAGES.AddMessage("Coeficientes: {}".format(classifier.coef_))
        MESSAGES.AddMessage("Intercepto: {}".format(classifier.intercept_))
        plt.bar(train_regressors_name, classifier.coef_[0])
        # Configurações adicionais
...
#Felipe 29/05/23
    ...
    if classifier_name == "Random Forest":
        ...
        plt.figure(figsize=(10, 6))
        plt.title("Importância dos Recursos")
        plt.bar(range(train_regressors.shape[1]), importances[np.argsort(importances)], align='center')
        #plt.xticks(range(train_regressors.shape[1]), train_regressors_name[indices], rotation=90)
        plt.xlabel("Mapas evidenciais")
        plt.ylabel("Importância")
        plt.tight_layout()
        plt.show()
